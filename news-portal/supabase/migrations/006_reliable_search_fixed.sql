-- Reliable Search Implementation (Fixed for Supabase)
-- Using triggers instead of generated columns due to unaccent immutability issue
...
alter table articles add column if not exists tags_norm text;
...
as $$
begin
  -- Update normalized fields (using plain SQL without unaccent in stored columns)
  NEW.title_norm := normalize_text(NEW.title);
  NEW.excerpt_norm := normalize_text(NEW.excerpt);
  NEW.tags_norm := normalize_text(NEW.tags);
...
create trigger update_article_search_trigger
  before insert or update of title, excerpt, tags, content
  on articles
  for each row
...
drop index if exists idx_articles_tags_norm_trgm;
...
-- 8. Create improved search function with unaccent in query
  ...
  with candidates as (
    select
      ...
      greatest(
        ...
        similarity(lower(unaccent(coalesce(a.tags, ''))), q_normalized)
      ) as trigram_score,
    ...
    where
      ...
      and (
        ...
        or lower(unaccent(coalesce(a.tags, ''))) % q_normalized
        -- Prefix matching
        or exists (
...
-- 11. Set trigram similarity threshold
